                ...
                <v-card-text>
                    <v-row class="mb-4" justify="space-between">
                        <v-col class="text-left">
                            ...
                            <v-fade-transition>
                                <v-avatar v-if="isPlaying" :color="color"
                                    :style="{
                    ...
                    <v-slider v-model="bpm" :color="color" track-color="grey" min="0" max="1" :step="0.01">
                        <template v-slot:prepend>
                            <v-btn size="small" variant="text" :color="color" @click="decrement">-</v-btn>
                        </template>
                        <template v-slot:append>
                            <v-btn size="small" variant="text" :color="color" @click="increment">+</v-btn>
                        </template>
                    </v-slider>
                </v-card-text>
            </v-card>
...
export default {
    ...
    methods: {
        ...
        get_data() {
            this.$ajax.get("/api/v1/pid/info").then(data => {
                return data.data;
            })
            .catch(error => {
                console.log(error);
            });
            //let y = Math.sin(this.t);
            //this.t += 0.1
            //return {aim: 0.5, speed: y, error: Math.random(), pwm: Math.random()};
        }
